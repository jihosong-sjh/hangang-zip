...
if [[ -z "${VITE_KAKAO_MAP_JS_KEY:-}" ]]; then
  echo "VITE_KAKAO_MAP_JS_KEY environment variable is required."
  exit 1
fi
...
VITE_PARK_DATA_SOURCE=api VITE_API_BASE_URL="$DOMAIN" VITE_KAKAO_MAP_JS_KEY="$VITE_KAKAO_MAP_JS_KEY" npm run build
